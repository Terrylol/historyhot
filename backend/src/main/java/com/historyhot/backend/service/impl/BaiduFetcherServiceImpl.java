...
/**
 * Service to fetch trending searches from Baidu
 */
@Service
@RequiredArgsConstructor
@Slf4j
public class BaiduFetcherServiceImpl implements PlatformFetcherService {

    private static final String PLATFORM_NAME = "baidu";
    ...
    public List<TrendingSearch> fetchTrendingSearches(Platform platform) {
        log.info("Fetching trending searches from Baidu");
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                ...
                if (cards.isArray() && cards.size() > 0) {
                    JsonNode hotList = cards.get(0).path("content");
                    
                    if (hotList.isArray()) {
                        int rank = 1;
                        for (JsonNode item : hotList) {
                            String title = item.path("query").asText();
                            String url = item.path("url").asText();
                            Long hotValue = item.path("hotScore").asLong();
                ...
                log.error("Failed to fetch Baidu trends. Response code: {}", response.getStatusCode());
            }
        } catch (Exception e) {
            log.error("Error fetching trending searches from Baidu: {}", e.getMessage(), e);
        }
